...
Return `true` if the secondary structure code of the given `el` is "C" "T", "S", or " ", indicating that it is a coil or loop region.
"""
function coilselector(el::Union{Residue,Atom})
    sscode = ss_code(el)
    return sscode == "C" || sscode == "T" || sscode == "S" || sscode == " "
end
