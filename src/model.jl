...
"""
function specialize_histidine!(r::Residue)
    if resname(r) == "HIS"
...
specialize_histidine!(r::DisorderedResidue) = foreach(specialize_histidine!, collectresidues(r; expand_disordered=true))
specialize_histidine!(c::Chain) = foreach(specialize_histidine!, c)
specialize_histidine!(m::Model) = foreach(specialize_histidine!, m)
specialize_histidine!(s::MolecularStructure) = foreach(specialize_histidine!, s)
