...
const pdbextension =
    Dict{Type,String}(PDB => "pdb", PDBXML => "xml", MMCIF => "cif", MMTF => "mmtf")
...
function pdbrecentchanges()
    ...
    modifiedlist =
        pdbstatuslist("ftp://ftp.wwpdb.org/pub/pdb/data/status/latest/modified.pdb")
    obsoletelist =
        pdbstatuslist("ftp://ftp.wwpdb.org/pub/pdb/data/status/latest/obsolete.pdb")
    ...
    if isfile(pdbpath) && !overwrite
        ...
        try
            ...
            if ba_number == 0
                if format == MMTF
                    Downloads.download(
                        "http://mmtf.rcsb.org/v1.0/full/$pdbid.mmtf.gz",
                        archivefilepath,
                    )
                else
                    Downloads.download(
                ...
                elseif format == MMCIF
                    ...
                    throw(
                        ArgumentError(
                            "Biological assemblies are available in the PDB and mmCIF formats only",
                        ),
                    )
                end
            end
            ...
            if !isfile(pdbpath) || filesize(pdbpath) == 0
                if format == PDB
                    throw(
                        ErrorException(
                            "Error downloading file: $pdbid; some PDB entries are not available as PDB format files, consider downloading the mmCIF file or MMTF file instead",
                        ),
                    )
                else
...
- `overwrite::Bool=false`: if set `true`, overwrites the PDB file if it exists
    in `dir`; by default skips downloading the PDB file if it exists.
"""
function downloadallobsoletepdb(;
    ...
    for line in eachline(input)
        ...
        elseif startswith(line, "MODEL ")
            try
                ...
                throw(PDBParseError("could not read model serial number", line_n, line))
            end
...
function spaceatomname(at::Atom)
    ...
    if strip_el == "" || findfirst(isequal(strip_el[1]), at_name) == nothing
        ...
        cent_ind =
            something(findfirst(isequal(strip_el[1]), at_name), 0) + length(strip_el) - 1
    end
    if cent_ind > 2
...
function checkchainerror(el::Union{Chain,AbstractResidue,AbstractAtom})
    if length(chainid(el)) != 1
        throw(
            ArgumentError(
                "Cannot write valid PDB file with non-single " *
...
function writepdb(
    ...
    open(filepath, "w") do output
        writepdb(output, el, atom_selectors...; expand_disordered = expand_disordered)
    end
end
...
function writepdb(
    ...
    if length(el) > 1
        for mod in sort(collect(el))
            ats =
                collectatoms(mod, atom_selectors...; expand_disordered = expand_disordered)
            if length(ats) == 0
                continue
            end
...
function writepdb(
    ...
    for at in collectatoms(el, atom_selectors...; expand_disordered = expand_disordered)
        checkchainerror(at)
